...
- (id) initWithFrame:(CGRect)aFrame minValue:(float)aMinValue maxValue:(float)aMaxValue barHeight:(float)height
{
    ...
    if (self)
	{
        ...
        sliderBarWidth = self.frame.size.width / self.transform.a - 5;
        
		self.minHandle = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"筛选滑块.png"] highlightedImage:[UIImage imageNamed:@"handle_highlight.png"]] autorelease];
		self.minHandle.center = CGPointMake(sliderBarWidth * 0.1, sliderBarHeight * 0.5);
		[self addSubview:self.minHandle];
		
		self.maxHandle = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"筛选滑块.png"] highlightedImage:[UIImage imageNamed:@"handle_highlight.png"]] autorelease];
		self.maxHandle.center = CGPointMake(sliderBarWidth * 0.9, sliderBarHeight * 0.5);
...
+ (id) doubleSlider
{
	return [[[self alloc] initWithFrame:CGRectMake(0., 0., 300., 80.) minValue:0.0 maxValue:1000.0 barHeight:10.0] autorelease];
}
