...
@synthesize imView;
@synthesize User_ID,Password,mArray;
@synthesize DengLu;
@synthesize delegate;
@synthesize cString,strat;
...
-(void)httpDengLuAsi
{
    ...
    if (strat == YES) {
        ...
        requestqq = [ASIHTTPRequest requestWithURL:urll];
        [requestqq setTag:1000];
        [requestqq setDelegate:self];
        [requestqq startAsynchronous];
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (request.tag==1000) {
        ...
        if (i > 200) {
            self.mArray = [RemDengLu recommendedParser:[request responseString]];
            
            if ([self.mArray count] != 0) {
                DengLuXML * deng = [self.mArray objectAtIndex:0];
                NSLog(@"  %@",deng.RESULT);
                ...
                [delegate stringWith:deng.RESULT With:User_ID.text WithArray:self.mArray];
                DengLu = [self.mArray objectAtIndex:0];
...
-(void)btn:(UIButton *)button
{
    ...
    if (strat == YES) {
        ...
        [requestqq clearDelegatesAndCancel];
